    ...
    <div class="container" style="min-height: 300px;">
      ...
      if ($sql_cart->num_rows > 0) {
        ...
        foreach ($sql_cart as $cart) { ?>
          <div class="container">
            <div class="row border-top border-bottom d-flex justify-content-center">
              <div class="row main align-items-center">
                <div class="col-2"><img class="img-fluid" src="images/produk/<?= $cart['gambar_produk']; ?>"></div>
                <div class="col">
                  <div class="row text-muted"><?= $cart['jenis']; ?></div>
                  <div class="row"><?= $cart['nama_produk']; ?></div>
                </div>
                <div class="col">
                  <?= $cart['qty_cart']; ?>
                </div>
                <div class="col">Rp. <?= number_format($total_harga = $cart['harga_produk'] * $cart['qty_cart']); ?> ,- <a href="delete_cart.php?id=<?= $cart['id_cart']; ?>"><span class="close">&#10005;</span></div></a>
              </div>
            </div>
          </div>
        <?php
        ...
        <div class="row">
          <div class="col-md mt-4 d-flex justify-content-end">
            <h4>Jumlah Bayar : Rp. <?= number_format($jumlah_bayar); ?> ,-</h4>
          </div>
          <br>
        </div>
        <div class="col-md mx-2 my-2  d-flex justify-content-end">
          <a href="index.php" class="btn btn-outline-secondary mx-2">Halaman Utama</a>
          <a href="checkout.php" class="btn btn-outline-info">Proses Checkout</a>
        </div>
      <?php } else { ?>
