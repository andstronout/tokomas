...
if (isset($_POST["submit"])) {
  if (!isset($_SESSION['login_pelanggan'])) {
    ...
    $sql_cart = sql("SELECT * FROM cart WHERE id_user='$_SESSION[id_pelanggan]' AND id_produk='$_POST[id_produk]'");
    $cart = $sql_cart->fetch_assoc();
    if ($sql_cart->num_rows > 0) {
      $tambah = $cart['qty_cart'] + $_POST["qty_cart"];
      $update_cart = sql("UPDATE cart SET qty_cart='$tambah' WHERE id_produk='$_POST[id_produk]'");
      echo "
        <script>
      ...
      $tambah = sql("INSERT INTO cart (id_user,id_produk,qty_cart) VALUES ('$_SESSION[id_pelanggan]','$_POST[id_produk]','$_POST[qty_cart]')");
      echo "
      <script>
      alert('Produk Ditambahkan, Check keranjangan belanja untuk proses checkout');
      document.location.href = 'index.php';
            ...
            <ul class="navbar-nav">
              ...
              <li class="nav-item">
                <a class="nav-link" href="cart.php">
                  ...
                  $cart = 0;
                  if (isset($_SESSION['login_pelanggan'])) {
                    $result = sql("SELECT * FROM cart WHERE id_user='$_SESSION[id_pelanggan]'");
                    if ($result) {
